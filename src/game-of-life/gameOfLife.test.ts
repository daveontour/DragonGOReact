...
describe("gameOfLife", () => {
  ...
  it("clamps cell pixel size to the supported range", () => {
    expect(clampCellPixelSize(0)).toBe(1);
    expect(clampCellPixelSize(99)).toBe(20);
  });

  it("measures grid dimensions from the viewport with a minimum cell size", () => {
    expect(measureGridDimensions(900, 600)).toEqual({ cols: 300, rows: 200 });
    expect(measureGridDimensions(100, 80)).toEqual({ cols: 48, rows: 32 });
  });

  it("preserves overlapping cells when resizing the grid", () => {
    const grid = setCells(4, 4, [
      [1, 1],
      [2, 2],
    ]);
    const resized = resizeGrid(grid, 4, 4, 6, 5);
    expect(getCell(resized, 6, 5, 1, 1, false)).toBe(1);
    expect(getCell(resized, 6, 5, 2, 2, false)).toBe(1);
    expect(getCell(resized, 6, 5, 5, 4, false)).toBe(0);
  });
});
