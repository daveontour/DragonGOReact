...
export function prepareCells(
  ...
  const turns = calculateTurns(rc.NumberFolds);
  let cells: Cell[];
  let minRow: number, minCol: number, maxRow: number, maxCol: number;

  if (rc.CellType.includes("knuth")) {
    const ck = calcCellsKnuth(turns, rc.StartDirection);
    cells = ck[0];
    minRow = ck[1];
    minCol = ck[2];
    maxRow = ck[3];
    ...
    const cc = calcCells(turns, rc.StartDirection);
    cells = cc[0];
    minRow = cc[1];
    minCol = cc[2];
    maxRow = cc[3];
    ...
    for (let i = 0; i <= height + 2; i++) {
      ...
      for (let j = 0; j <= width + 2; j++) {
        const cell: Cell = {
          ...
          SetAbsoluteCornersWithGrouting: function (): void {
            throw new Error("Function not implemented.");
          },
        };
